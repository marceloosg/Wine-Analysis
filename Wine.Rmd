...
ggplot(mwine,aes(x=value,fill=type))+geom_histogram()+facet_wrap(~variable)
```
...
## How many components we need to group similar inputs?
```{r,cache=T}
r=prcomp(wine[,1:12],scale=T)
sum(cumsum(r$sdev^2/sum(r$sdev^2)) <= 0.9)
```
