...
ggplot(wine[type=="White",],aes(x=alcohol,fill=quality))+geom_density()+facet_wrap(~quality)
...
ggplot(wine[type=="White",],aes(x=density,fill=quality))+geom_density()+facet_wrap(~quality)
...
ggplot(wine[type=="White",],aes(x=`volatile acidity`,fill=quality))+geom_density()+facet_wrap(~quality)
...
mad
```
The results are compatible with the Multiple Regression present at [Modeling Wine Preferences from Physicochemical Properties using Fuzzy inductive reasoning](http://www.scitepress.org/Papers/2015/55519/55519.pdf) and are within expected values for performance within the trained set, e. g. it have not overfitted.
