...
### What is the correlation between the variables? (bivariate analysis)
```{r,cache=T}
library(GGally)
...
results=fit(pcdata[,-c("PC7","PC6")], 'lm')
(results$train)
```

The results are compatible with the Multiple Regression present in the Fuzzy reference.
...
Review:
It is clear that we have 3 separable regimes, this perception is confirmed by the number of group chosen at the [Fuzzy inductive reasoning](http://www.scitepress.org/Papers/2015/55519/55519.pdf) paper.

The discretization process is a crutial part of the FIR algorithm. Maybe we could use Linear Discriminant Analysis to find the separation points for each input variable. The dimensionality should not be a problem if we use PCA before LDA.
```{r,warning=F}
library(caret)
library(MASS)
training=wine[type=="White"]
...
#train_ind=createDataPartition(training$group,p=0.7,list=F,groups = training$group)
...
source("fitPca.R")
lda=fitLda(traindata)
predicted=predict(lda,newdata = testdata)
tested=testdata$group
table(predicted,tested)
sum(predicted==tested)
predicted=predict(lda,newdata = testdata,type="prob")
predicted
data.table(predicted)
p=data.table(predicted)
p$index=seq(1:nrow(p))
melt(p,id.vars = "index")
mp=melt(p,id.vars = "index")
mp$variable
mp$variable[1]
as.integer(mp$variable[1])
as.integer(mp$variable[1])
mp
mp[,.(y=sum(as.integer(variable)*value)),index]
